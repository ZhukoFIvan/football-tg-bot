...
│   │   └── models.py         # Модели БД
│   ├── payments/             # Платежные провайдеры (FreeKassa, PayPalych)
...
```env
# FreeKassa (https://www.freekassa.ru/)
...
- `order_id` - FK на orders
- `user_id` - FK на users
- `payment_id` - уникальный ID платежа в системе провайдера
- `provider` - провайдер оплаты (freekassa, paypalych)
- `payment_method` - способ оплаты (card, sbp)
- `amount` - сумма платежа
- `currency` - валюта (RUB)
- `status` - статус платежа (pending, success, failed, cancelled, refunded)
- `payment_url` - URL для оплаты
- `description` - описание платежа
- `created_at`, `updated_at` - временные метки
- `paid_at` - дата и время успешной оплаты
...
**Важно:** Если вы добавляете модель `Payment` впервые, создайте миграцию:

```bash
docker compose exec api alembic revision --autogenerate -m "Add payments table"
docker compose exec api alembic upgrade head
```
...
Проект поддерживает оплату через **FreeKassa** и **PayPalych** с возможностью оплаты банковской картой или через СБП (Система быстрых платежей).
...
1. **Регистрация и получение данных:**
   ...
   - Зарегистрируйтесь на [FreeKassa.ru](https://www.freekassa.ru/)
   - В личном кабинете найдите:
     - `MERCHANT_ID` (ID магазина)
     - `Secret Key 1` (первый секретный ключ)
     - `Secret Key 2` (второй секретный ключ для webhook'ов)
...
3. **Настройка webhook в личном кабинете FreeKassa:**
   - Перейдите в раздел "Настройки" → "URL оповещения"
   - Укажите URL: `https://ваш-домен.com/api/payments/webhook/freekassa`
   - Метод: `POST`
   - Сохраните настройки
...
3. **Настройка webhook в личном кабинете PayPalych:**
   - Перейдите в раздел настроек webhook'ов
   - Укажите URL: `https://ваш-домен.com/api/payments/webhook/paypalych`
   - Метод: `POST`
   - Сохраните настройки

### Создание платежа

После добавления товаров в корзину, создайте платеж через API:
...
{
  "provider": "freekassa",  // или "paypalych"
  "payment_method": "card", // или "sbp" для СБП
  "promo_code": "DISCOUNT10", // опционально
  "bonus_to_use": 0 // опционально, количество бонусов для списания
}
```
...
```json
{
  "payment_id": "freekassa_123_abc12345",
  "payment_url": "https://pay.freekassa.ru/?m=...",
  "order_id": 123,
  "amount": 1999.00,
  "status": "pending"
```
...
Система автоматически обрабатывает уведомления от платежных систем:

- **FreeKassa:** `POST /api/payments/webhook/freekassa`
- **PayPalych:** `POST /api/payments/webhook/paypalych`

**Важно:** Эти эндпоинты должны быть доступны из интернета для получения уведомлений от платежных систем.

### Уведомления в Telegram

При изменении статуса платежа пользователь автоматически получает уведомление в Telegram бот:

- ✅ **Успешная оплата** - "Платеж успешно выполнен!"
- ❌ **Ошибка оплаты** - "Ошибка при оплате"
- ❌ **Отмена платежа** - "Платеж отменен"
...
1. **Создание платежа:**
   ...
   - Пользователь оформляет заказ из корзины
   - Создается заказ со статусом `pending`
   - Создается платеж через выбранный провайдер
   - Товары резервируются на складе
   - Корзина очищается
   - Возвращается `payment_url` для оплаты

2. **Обработка webhook:**

   - Платежная система отправляет уведомление на webhook
...
2. **Проверка webhook (тестовый запрос):**

   ```bash
   # Для FreeKassa (требует реальные данные от FreeKassa)
   curl -X POST "http://localhost:8000/api/payments/webhook/freekassa" \
     -H "Content-Type: application/x-www-form-urlencoded" \
     -d "MERCHANT_ID=...&AMOUNT=100&MERCHANT_ORDER_ID=1&SIGN=..."
   ```
...
- Проверяйте подписи всех входящих webhook'ов

⚠️ **Тестирование:**

- Используйте тестовые режимы платежных систем для разработки
- Проверяйте обработку всех статусов (success, failed, cancelled)
- Убедитесь, что уведомления доставляются пользователям

⚠️ **Мониторинг:**

- Следите за логами webhook'ов
- Проверяйте статусы платежей в базе данных
- Настройте алерты на ошибки платежей

### Статистика платежей

Статистику всех платежей можно получить через админ-панель бота:

- В Telegram боте: `/admin` → "💳 Статистика платежей"
- Через API: `GET /api/admin/stats/payments`
...
- [x] Интеграция платежей (FreeKassa, PayPalych)
- [x] Админ-панель для управления товарами
- [x] Система заказов и истории покупок
- [x] Система скидок и промокодов
- [x] Статистика платежей
- [x] Уведомления о платежах в Telegram
