...
async def payment_success_page(order_id: int, db: AsyncSession = Depends(get_db)):
    """
    Страница успешной оплаты - перенаправляет обратно в мини-апп
    """
    logger.info(f"Payment success page accessed for order #{order_id}")
    ...
    if order:
        order_info = f"Заказ #{order.id} на сумму {float(order.final_amount):.2f} ₽"
    else:
        order_info = f"Заказ #{order_id}"
    
    # HTML страница с автоматическим редиректом в мини-апп
    html_content = f"""
<!DOCTYPE html>
<html lang="ru">
<head>
...
<body>
    <div class="container">
        ...
        <p>Ваш заказ успешно оплачен и отправлен в обработку.</p>
        <div class="redirect-info">
            Возвращаем вас в приложение<span class="spinner"></span>
        </div>
    </div>
    
    <script>
        // Редирект обратно в мини-апп через настроенное время
        setTimeout(function() {{
            // Если открыто в Telegram WebApp
            if (window.Telegram && window.Telegram.WebApp) {{
                window.Telegram.WebApp.close();
            }} else {{
                // Альтернативный редирект на фронтенд
                window.location.href = '{settings.FRONTEND_URL}/orders/{order_id}';
            }}
        }}, {settings.PAYMENT_SUCCESS_REDIRECT_TIMEOUT});
    </script>
</body>
</html>
    """
...
async def payment_fail_page(order_id: int, db: AsyncSession = Depends(get_db)):
    """
    Страница неудачной оплаты - перенаправляет обратно в мини-апп
    """
    logger.info(f"Payment fail page accessed for order #{order_id}")
...
<body>
    <div class="container">
        ...
        <p>Вы можете попробовать оплатить заказ снова в личном кабинете.</p>
        <div class="redirect-info">
            Возвращаем вас в приложение<span class="spinner"></span>
        </div>
    </div>
    ...
    <script>
        ...
        }}, {settings.PAYMENT_FAIL_REDIRECT_TIMEOUT});
    </script>
</body>
</html>
    """
